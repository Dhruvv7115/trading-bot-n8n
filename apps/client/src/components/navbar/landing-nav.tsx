...
export default function LandingNav() {
	...
	return (
      ...
      {/* web navbar */}
			<div className="hidden md:flex items-center justify-between w-full px-6 py-4 gap-4 border border-border bg-background/20 backdrop-blur-md rounded-lg">
				...
				<div className="flex items-center gap-6">
					<motion.button
						whileTap={{ scale: 0.95 }}
						onClick={() => toggleTheme()}
						className="hover:bg-accent hover:text-accent-foreground text-muted-foreground transition-colors rounded-[12px] p-2 border"
					>
						{theme === "light" ? (
					...
					<div className="flex items-center gap-2">
						...
						<Button className="rounded-[12px]" variant="outline">Get Started</Button>
					</div>
				</div>
			</div>
      {/* mobile navbar */}
